...
def parse_registration_point(file_path, display_results):
    data = []
...
def resample_center_line(center_line, display_results, z_distance):
...
def find_centerline_vectors(center_line_pp, display_results):
...
def find_closest_point_index(point_cloud, target_point):
...
def register_OCT_frames_onto_centerline(grouped_OCT_frames, centerline_registration_start, centerline_vectors,
    ...
    for z_level, frame_points in grouped_OCT_frames.items():
        ...
        rotation_matrix = rotation_matrix_from_vectors(source_normal_vector, normal_vector)
...
def get_oct_lumen_rotation_matrix(centerline_registration_start, grouped_OCT_frames, registration_point_OCT, registration_point_CT, OCT_registration_frame, z_distance, display_results):
    ...
    rotation_matrix = rotation_matrix_from_vectors(vector_oct_registration_point, vector_ct_registration_point)
...
def rotation_matrix_from_vectors_x_y(vec1, vec2):
    # Normalize the input vectors
    vec1 = vec1 / np.linalg.norm(vec1)
    vec2 = vec2 / np.linalg.norm(vec2)
...
def rotate_frames(grouped_OCT_frames, oct_lumen_rotation_matrix):
...
def smooth_centerline(pc_centerline, display_results):
    point_start = parse_marked_point("workflow_data/location_2.txt")
    point_end = parse_marked_point("workflow_data/location_1.txt")
...
if __name__ == "__main__":
    # Displaying and saving settings
    display_results = False
    save_file = True
    z_distance = 0.1

    # Parse OCT frames
    grouped_OCT_frames = parse_OCT_lumen_point_cloud("workflow_processed_data_output/output_point_cloud.txt")

    # Parse center line point cloud
    #pc_centerline = parse_point_cloud_centerline("workflow_data/centerline_phantom.txt", display_results)

    # Make smoother bifurication curves
    # smoothed_pc_centerline = smooth_centerline(pc_centerline, display_results)
    ...
    resampled_pc_centerline = resample_center_line(smoothed_pc_centerline, display_results, z_distance)

    # Create a 3D scatter plot for the original points
    # Compute center line vectors, that point from the current point to the next point
    centerline_vectors = find_centerline_vectors(resampled_pc_centerline, display_results)

    # Find marked registration point in CT
    registration_point_CT = parse_registration_point("phantom_data/registration_point_CT.txt", display_results)

    # Find marked registration point in OCT
    registration_point_OCT = parse_registration_point("workflow_processed_data_output/aligned_OCT_registration_point.txt", display_results)

    OCT_registration_frame = 373 - 1  #121
    # Adapt z value
    centerline_registration_start = find_closest_point_index(resampled_pc_centerline, registration_point_CT)
    if display_results:
    ...
    oct_lumen_rotation_matrix, rotated_registration_point_OCT = get_oct_lumen_rotation_matrix(centerline_registration_start, grouped_OCT_frames, registration_point_OCT, registration_point_CT, OCT_registration_frame, z_distance, display_results)

    # rotate OCT_frames
    rotated_grouped_OCT_frames = rotate_frames(grouped_OCT_frames, oct_lumen_rotation_matrix)
    ...
    register_OCT_frames_onto_centerline(rotated_grouped_OCT_frames, centerline_registration_start, centerline_vectors,
                                        resampled_pc_centerline, OCT_registration_frame, z_distance, rotated_registration_point_OCT, save_file, display_results)
