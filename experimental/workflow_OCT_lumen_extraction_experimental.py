...
def icp_alignment(first_contour, points, reference_points, transformation_matrix_previous, display_images, registration_point, max_iterations=100, distance_threshold=100, convergence_translation_threshold=1e-3,
    ...
    for iter_num in range(max_iterations):
        ...
        if (abs(closest_rot_angle) < convergence_rotation_threshold) \
            ...
            if False:
                plt.plot(points[:, 0], points[:, 1], 'x')
                plt.plot(reference_points[:, 0], reference_points[:, 1], 'o')
                plt.show()

            break
    if False:
        plt.plot(reference_points[:, 0], reference_points[:, 1], 'x', color = "red")
        plt.plot(points[:, 0], points[:, 1], 'x', color="blue")    
        plt.plot(source_points_orig[:, 0], source_points_orig[:, 1], 'x', color="green")
        plt.plot(first_contour[:, 0], first_contour[:, 1], 'x', color="black")
        plt.show()
    transformation_matrix = compute_transformation_matrix(source_points_orig, points)
...
def icp_alignment_old(first_contour, source_spline, target_spline, transformation_matrix_previous, display_images, max_iterations=100, rotation_limit=2):
    """
    ...
    # Rough initial alignment
    if False: #transformation_matrix_previous is not None:
        # Pad the 3D point cloud with a column of 1's.
        plt.plot(target_points[:, 0], target_points[:, 1], 'x', color = "red")
    ...
    first_contour
    plt.plot(target_points[:, 0], target_points[:, 1], 'x', color = "red")
    plt.plot(source_points[:, 0], source_points[:, 1], 'x', color="blue")    
    plt.plot(source_points_orig[:, 0], source_points_orig[:, 1], 'x', color="green")
    plt.plot(first_contour[:, 0], first_contour[:, 1], 'x', color="black")
    plt.show()
        ...
        for page in range(im.n_frames):
            ...
            if current_contour is not None:
                ...
                if previous_contour is not None:
                    ...
                    aligned_source_spline, transformation_matrix, final_rotation, total_trans_x, total_trans_y, updated_registration_point = icp_alignment(first_contour, current_contour, previous_contour, transformation_matrix_previous, display_images, registration_point)
...
if __name__ == "__main__":
    ...
    z_offset = 0.1  # Increment by 0.2 or 0.1mm
